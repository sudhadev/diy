...
foreach($_FILES as $fieldname=>$arrValues){

	$fname= $arrValues['name'];
	$fTmp = $arrValues['tmp_name'];
	...
	//$errNumber[$fieldname]=$objftp_img->show_error_string();
	$errNumber=$objftp_img->show_error_string(); //echo $errNumber."+++=";
	if($errNumber) {
		$msg=array('ERR',$errNumber);
		$msgText.= "<strong>$fname:</strong> ". str_replace("{%MAX_SIZE%}",$objCore->_SYS['CONF']['F_SIZE_PRINT']['IMAGE'],$objCore->_SYS['MSGS']['IMG_UPLOAD'][$msg[1]][1])."<br/>";
	
	}else{
		$imageName=$renTo.''.$objftp_img->imgExt;$allImages.=$imageName."|i|";
		echo '<img src="'.$objCore->_SYS['CONF']['URL_IMAGES_LISTINGS']."/".$listUser."/thumbs/".$imageName.'" border="1" width="65" style="padding:3px;" />';
	}
...
<script language="Javascript" type="text/javascript">
    window.parent.passImgKey('<?php echo $allImages;?>');
    window.parent.document.getElementById('zooming').style.display='none';
    window.parent.document.getElementById('imgNames').value='<?php echo $allImages;?>';
    </script>
    <?php }?>
