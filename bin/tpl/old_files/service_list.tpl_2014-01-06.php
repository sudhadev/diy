<?php
        ...
        {
            $serviceList = $objSearch->getServiceList($_REQUEST['keyword'], $_REQUEST['latitude'], $_REQUEST['longitude'], $_REQUEST['radius'], $_REQUEST['pg'], $_REQUEST['categoryId'], $url, $_REQUEST['resultsPerPage'], $_REQUEST['order_by']);
        }
     ...
     ?>
              ...
              <tr>
                ...
                <div id="add_selections">
                    ...
                    <!--<img onclick="search_result_services.submit();" src="<?php echo $objCore->_SYS['CONF']['URL_IMAGES_FRONT'];?>/search_page/wish_list_button.jpg" width="170" height="20" border="0" />-->
                </div>                </td>
              </tr>
              <tr>
			  	<td>
...
<!--           <img src="<?php echo $objCore->_SYS['CONF']['URL_IMAGES_FRONT'];?>/search-icon.jpg" width="14" height="15" /> 
           <a href="javascript: zoom('<?php echo $objCore->_SYS['CONF']['URL_IMG_UPLOAD_MODULE'];?>/zoom_prods.php','<?php echo $serviceList[0][11]; ?>','services');">Enlarge Image</a>-->
               
<?php
echo '<a href="'.$objCore->_SYS['CONF']['URL_FRONT'].'browse/?f=more&catid=2&cid='.$serviceList[$i][6].'&lid='.$serviceList[$i][20].'&dis='.round($serviceList[$i][7], 2).'&unit=miles">';

        $image_array = array($serviceList[$i][11],$Classifiedlist[$i][22],$Classifiedlist[$i][23],$Classifiedlist[$i][24]);
        ...
        <div class="classified_description_wraper">
			<div class="select_wishlist">
			  <div class="where_text common_text_ash"> Where are they</div>
				<div class="select_wishlist_button"><img height="15" width="15" alt="where are they?" class="cursorHand" src="<?php echo $objCore->_SYS['CONF']['URL_IMAGES_FRONT'];?>/where_icon.jpg" onClick="JavaScript:PopupWhereAreThey('<?php echo $objCore->_SYS['CONF']['URL_FRONT'];?>/search/where_are_they.php?cid=<?php echo $serviceList[0][6]; ?>');"/></div>
			</div>
            <div class="description_text_big">
            <?php
             echo '<a class="heading_link" href="'.$objCore->_SYS['CONF']['URL_FRONT'].'browse/?f=more&catid=2&cid='.$serviceList[$i][6].'&lid='.$serviceList[$i][20].'&dis='.round($serviceList[$i][7], 2).'&unit=miles" style="font-family: Arial,Helvetica,sans-serif;color: #333333;
    font-size: 16px;
    font-weight: bold;">';
            ?>
    ...
    echo ucwords($serviceList[$i][15]); 
                                ?><br/>
        
        <?php echo $serviceList[$i][21]; ?><br/>
        <?php
        echo '</a>';
...
<!--					<div class="description_subdiv common_text_ash_bold">Area:</div>-->
					<div class="classified_desc_subsec common_text" style="padding-left: 0px;"><?php echo $serviceList[$i][12].", ".$serviceList[$i][13]; ?></div>
				</div>
				<div class="classified_desc_subdiv">
					<div class="description_text_big">
                                            <?php  echo '<a href="'.$objCore->_SYS['CONF']['URL_FRONT'].'browse/?f=more&catid=2&cid='.$serviceList[$i][6].'&lid='.$serviceList[$i][20].'&dis='.round($serviceList[$i][7], 2).'&unit=miles" style="font-family: Arial,Helvetica,sans-serif;color: #333333;
    font-size: 16px;
    font-weight: bold;">Services Offered:</a>'; ?><br/></div>
                                        <div class="classified_desc_subsec common_text" style="padding-left: 0px;">
                                        <?php
                                         if(strlen($list[$i][20])>80){
                                                        echo substr($serviceList[$i][9], 0, 130).'<span>...</span><a href="'.$objCore->_SYS['CONF']['URL_FRONT'].'browse/?f=more&catid=2&cid='.$serviceList[$i][6].'&lid='.$serviceList[$i][20].'&dis='.round($serviceList[$i][7], 2).'&unit=miles" style="text-decoration:underline;font-weight:bold;"> more</a>';
                                                    }
                                                    else{
                                                        echo $serviceList[$i][9].'<span>...</span><a href="'.$objCore->_SYS['CONF']['URL_FRONT'].'browse/?f=more&catid=2&cid='.$serviceList[$i][6].'&lid='.$serviceList[$i][20].'&dis='.round($serviceList[$i][7], 2).'&unit=miles" style="text-decoration:underline;font-weight:bold;"> more</a>';
...
<!--						<div class="classified_desc_subsec_sub"><a href="JavaScript:Popup('<?php echo $objCore->_SYS['CONF']['URL_FRONT'];?>/search/info.php?cid=<?php echo $serviceList[$i][6]; ?>');"><?php echo $serviceList[$i][0]." ".$serviceList[$i][1]; ?></a><br /><?php echo $serviceList[$i][2]; ?></div>-->
						<div class="classified_desc_subsec_sub">
							<div class="classified_desc_subsec common_text">Tel:&nbsp;<?php echo $serviceList[$i][2]; ?></div>
                                                        <?php if($serviceList[$i][26]!=''){ ?>
                                                        <div class="classified_desc_subsec common_text">Mob:&nbsp;<?php echo $serviceList[$i][26]; ?></div>
							</div>
                                                        <?php } ?>
                                                        <div class="classified_desc_subsec common_text">Email:&nbsp;<a href="mailto:<?php echo $serviceList[$i][3]; ?>"><?php echo $serviceList[$i][3]; ?></a></div>
                                        ...
                                        <div class="classified_desc_subsec common_text" style="margin-left: -4px;">
                                        <?php if($list[$i][25]!=""){?>
                                                    <a href="<?php echo "http://".str_replace("http://", '', $list[$i][25]);?>"><img src="<?php echo $objCore->_SYS['CONF']['URL_IMAGES_FRONT'];?>/visit-website.png"/></a>
                                                    <?php } ?> </div>
                                        
                                        </div>
            <div class="rate_distance_maindiv">
			<div class="hourly_rate_main">
				<div class="hourly_rate hourly_rate_font">Call out charge: </div>
				<div align="right" class="hourly_rate_amount hourly_rate_font"><?php if($serviceList[$i][10]==0)echo "No"; else echo "(£) ".$serviceList[$i][10]; ?></div>
			</div>
			<div class="distance_main">
				<div class="hourly_rate distance_font">Distance (<?php echo $unit; ?>) </div>
				<div align="right" class="hourly_rate_amount distance_font"><?php echo round($serviceList[$i][7], 2); ?></div>
			</div>
            ...
            <input type="checkbox" name="checkVal[<?php echo $i;?>]" id="checkVal[<?php echo $i;?>]" value="<?php echo $serviceList[$i][20];?>" onclick="javascript:selectItems('<?php echo $serviceList[$i][2010];?>');" style="    margin-top: -2px;
...
<!--        <div class="classified_description_sub_wraper">
        ...
        Notes:        </div>
        <div class="classified_description_subdesc common_text"><?php echo $serviceList[$i][9]; ?></div>
        </div>-->
<!--        			<div class="select_wishlist">
			  <div class="select_wishlist_text common_text_ash">Select for wish list</div>
				<div class="select_wishlist_button">

				<input type="checkbox" name="checkVal[<?php echo $i;?>]" id="checkVal[<?php echo $i;?>]" class="select_wishlist_button"  value="<?php echo $serviceList[$i][20];?>"/>
                                </div>
			</div>-->
		</div>
 </div>
