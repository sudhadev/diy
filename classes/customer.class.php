...
class Customer extends Sql {
    ...
    function __construct($gConf = '') {
        ...
        $this->loggedUserId;
        $this->core->sessCusId;
        if (strlen($this->loggedUserId) < 11)
        ...
        /*  elseif ($addressError[1] == true && $cusType == "S") {
            ...
            if ($result) {
                ...
                if ($cusType == "S") {
                    ...
                    if (!is_dir($baseDir)) {
                        mkdir($baseDir, 0775);
                        mkdir($baseDir . "/large/", 0775);
                        mkdir($baseDir . "/thumbs/", 0775);
                    }
            ...
            if ($result) {
                ...
                if ($cusType == "S") {
                    ...
                    if ($subscription == 'M') {
                        ...
                        $package_type_extend = $this->package_type_extend;
                    } else {
                        $package = '6';
                        $expire = time() + (60 * 60 * 24 * 30 * 6);
                        $package_type_extend = $this->package_type_extend;
                    }
                    $this->setSubcriptons($customerId, $subscription, $package, $expire, 'admin', $package_type_extend);

                    //$resultSubscription = $this->query($sqlSubscription);
                    if ($register_status == 'P') {
                        $objEmail = new Email();
                        $objEmail->send('register_supplier', $this->email, $customerId, 'H', $this->password);
                        $customerId = urlencode($customerId);
                        //$redirect =  $this->config['URL_FRONT'].'/signup/?f=verify&uid='.$customerId.'&email='.$this->email.'&ps='.$this->password.'&fr=acc';
                        //return array('SUC','DONE_SUPPLIER');
                        //header("Location: $redirect"); 
...
//                    $objSession->login($this->email, $this->password, 1,'','',$this->core->_SYS['CONF']['URL_MY_ACCOUNT']."/");
    ...
    function verify($cus_Id, $verify_code = '') {
        ...
        $sql = "SELECT email,password,ver_key,register_status,customer_type,expire_time,pass FROM `" . $this->tblPrefix . "customers` WHERE customer_id = '" . $cus_Id . "'";
...
//        exit();
        ...
        if ($result) {
            if ($result[0]['register_status'] == 'E') {
                ...
                $objSession = new Session();
                $objSession->config = $this->lConf;
                $objSession->login($result[0]['email'], $result[0]['pass'], 1, '', 'verification', $this->core->_SYS['CONF']['URL_MY_ACCOUNT'] . "/");
                ...
                if ($result[0]['customer_type'] == "S") {
                    ...
                    $objSession = new Session();
                    $objSession->config = $this->lConf;
                    $objSession->login($result[0]['email'], $result[0]['pass'], 1, '', 'verification', $this->core->_SYS['CONF']['URL_FRONT'] . "/");
                    return array('SUC', 'DONE_REGISTER');
                }
            } else {
...
function checkPassword2($password, $passwordConfirm='', $field) {
        if ($field == 'password') {
            if (empty($password)) {
                $msg = array('ERR', 'PW_BLANK');
                return array($msg, true);
    ...
    function checkEmail2($email, $emailConfirm='', $field) {
    	if ($field == 'email') {
    		if (empty($email)) {
    			$msg = array('ERR', 'EMAIL_BLANK');
    			return array($msg, true);
...
customers.mobile, customers.email, customers.show_website, customers.title,   customers.latitude, customers.longitude,customers.id, customers.website, customers.mon,customers.sat,customers.sun,customers.register_status,
...
WHERE customers.customer_id='" . $customer_id . "'";
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            $list[$i][] = $result[$i]['register_status']; //23
            $list[$i][] = $result[$i]['pass']; //24
        }
    ...
    function dList($where = '') {
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            $list[$i][] = $result[$i]['register_status']; // 8
        }
        return $list;
    }
    ...
    function deleteCustomer($customer_id) {
        ...
        $sql = "DELETE FROM `" . $this->tblPrefix . "customers` WHERE customer_id='" . $customer_id . "'";
        if ($this->query($sql)) {
...
//                        }
                        ...
                        // time should keep as it is
                        if (!$expire || $expire < time() || strlen($expire) < 10)
...
//						}
    ...
    /*
     * 	get active user IDs  - Create By Maduranga 
     * */

    function get_active_sup_ids($topId, $catId, $subCatId, $tblId = '', $time = '') {
        //$this->dev=true;
        ...
        if ($catId && $subCatId) {
            switch ($topId) {
                case 1: {
                        $sql = "SELECT  listing_materials.supplier_id as supplier_id
	    					FROM `" . $this->tblPrefix . "listing_materials`
	...
	`" . $this->tblPrefix . "manufacturers` manufacturers ON listing_materials.manufacturer_id=manufacturers.id JOIN `" . $this->tblPrefix . "categories` categories ON listing_materials.category_id_2=categories.id 
			JOIN `" . $this->tblPrefix . "customers` customers ON (listing_materials.supplier_id = customers.customer_id 
	            AND customers.customer_id IN ( SELECT `cus`.`customer_id`
	FROM `" . $this->tblPrefix . "customers`=`cus` , `" . $this->tblPrefix . "subscriptions`=`sub`
	WHERE `cus`.`customer_id`=`sub`.`customer_id` 
	AND `cus`.`customer_type`='S' 
	AND `sub`.`expire`>='" . time() . "' ) ) 
			WHERE (listing_materials.added_time between '" . $tfm . "' AND '" . $tto . "' )
	AND listing_materials.category_id_1='" . $catId . "' AND listing_materials.category_id_2='" . $subCatId . "' ";
                        if ($tblId) {
                            $sql.= " AND listing_materials.id='" . $tblId . "'";
                        }
                    }break;
                case 2: {
                        $sql = "SELECT  customers.customer_id as supplier_id
    						FROM `" . $this->tblPrefix . "services` services 
 				JOIN `" . $this->tblPrefix . "customers` customers ON (services.supplier_id = customers.customer_id
					...
					AND `sub`.`expire`>='" . time() . "' )
							...
							WHERE (services.added_date between '" . $tfm . "' AND '" . $tto . "') 
 								AND category_id_1='" . $catId . "' 
 										AND category_id_2='" . $subCatId . "'";
                ...
                case 3: {
                        $sql = "SELECT customers.customer_id as supplier_id
    						 FROM `" . $this->tblPrefix . "classified_ads` classads 
    						 		JOIN `" . $this->tblPrefix . "customers` customers ON (classads.supplier_id = customers.customer_id
            							AND customers.customer_id IN ( SELECT `cus`.`customer_id`
										FROM `" . $this->tblPrefix . "customers`=`cus` , `" . $this->tblPrefix . "subscriptions`=`sub`
										WHERE `cus`.`customer_id`=`sub`.`customer_id` 
										AND `cus`.`customer_type`='S' 
										AND `sub`.`expire`>='" . time() . "' )
    						 				...
    						 				WHERE (classads.added_date between '" . $tfm . "' AND '" . $tto . "')
												AND classads.category_id_1='" . $catId . "' 
														AND classads.category_id_2='" . $subCatId . "' $sqlHidModaratedList";
    ...
    function getListings($customerId, $topId, $catId, $subCatId, $tblId = '', $time = '', $pg = 1, $url = '') {
        ...
        if ($this->loggedUser != "Admin")
            $sqlHidModaratedList = "AND (listing_active='Y' OR listing_active='N')";
...
listing_materials.bulk_discount, listing_materials.bulk_price, listing_materials.delivery, listing_materials.id, listing_materials.supplier_code, listing_materials.listing_active, listing_materials.deact_reason, listing_materials.image as list_image, listing_materials.description as list_desc,categories.image, specifications.keywords , customers.company
                            		FROM `" . $this->tblPrefix . "listing_materials`
listing_materials JOIN `" . $this->tblPrefix . "specifications` specifications ON listing_materials.specification_id=specifications.id JOIN
`" . $this->tblPrefix . "manufacturers` manufacturers ON listing_materials.manufacturer_id=manufacturers.id JOIN `" . $this->tblPrefix . "categories` categories ON listing_materials.category_id_2=categories.id 
		JOIN `" . $this->tblPrefix . "customers` customers ON (listing_materials.supplier_id = customers.customer_id  ) 
...
AND listing_materials.category_id_1='" . $catId . "' AND listing_materials.category_id_2='" . $subCatId . "' $sqlHidModaratedList";
 ...
 FROM `" . $this->tblPrefix . "services` WHERE supplier_id='" . $customerId . "' AND category_id_1='" . $catId . "' AND category_id_2='" . $subCatId . "' $sqlHidModaratedList";
                            ...
                            $sql = "SELECT services.business_name, services.affiliations, services.price, services.call_out_charge,services.image, services.contact_person, services.accreditation, services.website, services.id, services.status, services.keywords, services.deact_reason , services.added_date ,
                            		 customers.customer_id, customers.title,customers.f_name,customers.l_name ,customers.company
...
AND classads.category_id_1='" . $catId . "' AND classads.category_id_2='" . $subCatId . "' $sqlHidModaratedList";
    ...
    function getListingCount($var) {
        ...
        if ($timeBetween)
            $sqlExtraSupplies = " ( listing_materials.added_time  $timeBetween ) AND ";
        if ($timeBetween)
            $sqlExtraServices = " ( added_date  $timeBetween ) AND ";
        if ($timeBetween)
            $sqlExtraClassified = " ( added_date  $timeBetween ) AND ";
...
// End - function
    ...
    function updatePasswordsBulk() {
        $customers = $this->dList();

        for ($i = 0; $i < count($customers); $i++) {
            $password = $this->generateRandomString(10);
            $hashed_pw = md5($password);
            $sql = "UPDATE `" . $this->tblPrefix . "customers` SET `password` = '$hashed_pw', `pass` = '$password' WHERE `customer_id` = '".$customers[$i][0]."'";
            $result = $this->query($sql);
        }
    }
    
    function getCustomersExpired() {
        $cur_time = time();
        $expire_extend = time()+(60*60*24*30);
        $sql = "UPDATE `" . $this->tblPrefix . "subscriptions` SET `expire` = '$expire_extend' WHERE expire <= '$cur_time'";
        $result = $this->query($sql);
        //return $result;
    }
    
    function getCustomerById($customer_id) {
        $sql = "SELECT * FROM `" . $this->tblPrefix . "customers` WHERE `id`='" . $customer_id . "'";
        $result = $this->query($sql);
        for ($i = 0; $i < count($result); $i++) {
            $list[$i][] = $result[$i]['f_name']; //0
            $list[$i][] = $result[$i]['l_name']; //1
            ...
            $list[$i][] = $result[$i]['mobile']; //10
            $list[$i][] = $result[$i]['email']; //11
            $list[$i][] = $result[$i]['title']; //12
            $list[$i][] = $result[$i]['latitude']; //13
            $list[$i][] = $result[$i]['longitude']; //14
            $list[$i][] = $result[$i]['id']; //15
            $list[$i][] = $result[$i]['mon']; //16
            $list[$i][] = $result[$i]['sat']; //17
            $list[$i][] = $result[$i]['sun']; //18
            $list[$i][] = $result[$i]['website']; //19
            $list[$i][] = $result[$i]['show_website']; //20
            $list[$i][] = $result[$i]['register_status']; //21
            $list[$i][] = $result[$i]['pass']; //22
            $list[$i][] = $result[$i]['customer_id']; //23
        }

        return $list;
    }
/*this function for sending email when a new customer successfully add*/    
/*public function sentRegSucsessEmail($email,$name,$title){
	
	$to = $email;
	$subject = "DIY Price Check Registation";
	$txt = "hi ".$title." ".$name.",<br>Your Registation is successfully compleated";
