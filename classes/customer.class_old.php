...
//                    $objSession->login($this->email, $this->password, 1,'','',$this->core->_SYS['CONF']['URL_MY_ACCOUNT']."/");
    ...
    function checkfName($fName) {
        ...
        else if ((preg_match ('/[^a-z ]/i', $fName))>0) {
    ...
    function checklName($lName) {
        ...
        else if ((preg_match ('/[^a-z ]/i', $lName))>0) {
...
//						}
    ...
    function changePassword($customerId, $password, $newPassword, $newPasswordConfirm) {
        ...
        if($result[0]["COUNT(*)"]>0) {
            ...
            elseif ($passwordMatchError[1]) {
                ...
                $encryptedPassword = $this->encryptPassword($newPassword);
                $sql = "UPDATE `".$this->tblPrefix."customers` SET password = '".$encryptedPassword."' WHERE customer_id='".$customerId."'";
                $this->query($sql);
    ...
    function setSubcriptons($customerId, $subscription='C', $package='',$expire='') {
        ...
        if ($result[0]['COUNT(*)'] == 0) {
            ...
            $sql = "INSERT INTO `".$this->tblPrefix."subscriptions` (`customer_id`, `subscription_type`, `package_type`, `date`, `last_modified`, `expire`, `no_of_listings`, `subscription_status`) VALUES ('".$customerId."', '".$subscription."', '".$package."', '".time()."', '', '".$expire."', '', '".$status."')";
            if ($this->query($sql)) {
                if ($subscription == 'C') {
