...
class Listing extends Sql {
    ...
    function add_edit($arrParentId,$logId,$unitCost,$bulkDiscount,$bulkPrice,$delivery,$listingActive,$img,$desc,$supplier_code,$list_spec,$del,$del_rate,$header,$url,$img2,$img3,$img4) {
        ...
        $bulkPrice = str_replace(',','',$bulkPrice);
        $bulkDiscount = str_replace(',','',$bulkDiscount);
        $bulkPrice = str_replace(',','',$bulkPrice);
        $del_rate = str_replace(',','',$del_rate);
        
        if($bulkPrice == "0") {
            $bulkPrice = "";
        }

        if((($unitCost == "" ||($bulkPrice == "" &&  $bulkDiscount!=0) )) && $listingActive=='Y') {
            //echo "came1";
            if($unitCost == ""){
                $msg=array('ERR','BLANK_UNIT_COST');
            }elseif($bulkPrice == "" &&  $bulkDiscount!=0){
                $msg=array('ERR','BLANK_BULK_DISCOUNT');
            }
        ...
        }elseif((!is_numeric((int)$unitCost) || ((int)$delivery!="" && !is_numeric((int)$del_rate)) || !is_numeric((int)$bulkDiscount) || $this->check_numeric((int)$bulkPrice)) && $listingActive=='Y') {
            $msg=array('ERR','NOT_NUMERIC');

        } elseif(((int)$unitCost < (int)$bulkPrice) && $listingActive=='Y') {
            $msg=array('ERR','NOT_GREATER');

        } elseif(($bulkPrice != "" && $bulkDiscount == 0)&& $listingActive=='Y') {
            ...
            if($list[$i][2] == $type) //subscription_type
            {
                ...
                //customer_status,subscription_status,no_of_listings
                {
                    ...
                    if($available_listings >= $need_to_add_listings) {
                        $return_val = false;//2015/07/14 ashan- if this make as false[original file contain false] when activate new file, return error message.
                    } else {
                        if($available_listings == 0) {
                            $objCustomer->setStatus($logId, 'E', $type, 'sub');//Ashan
                        }
                        $return_val = true;
                    }
                } else {
...
//                    }
    ...
    function getSpecifications($categoryId) {
        ...
        $sql = "SELECT DISTINCT(listing_materials.specification_id), listing_materials.manufacturer_id, specifications.specification, 
        		manufacturers.manufacturer FROM `".$this->tblPrefix."listing_materials` 
        				listing_materials JOIN `".$this->tblPrefix."specifications` specifications 
        						ON listing_materials.specification_id=specifications.id JOIN `".$this->tblPrefix."manufacturers` manufacturers 
        								ON listing_materials.manufacturer_id=manufacturers.id 
        								WHERE listing_materials.category_id_2 = '".$categoryId."'
											AND listing_materials.supplier_id IN (
        										SELECT `c`.`customer_id` 
        										FROM `".$this->tblPrefix."customers`=`c` , `".$this->tblPrefix."subscriptions`=`s` 
        										WHERE `c`.`customer_type`='S' 
        										AND `c`.`customer_id`=`s`.`customer_id` 
        										AND `c`.`customer_status`='Y' 
        										AND `s`.`subscription_type`='M' 
        										AND `s`.`subscription_status`='Y' 
        										AND `s`.`expire`>'".time()."' 
                    		                    )
        										AND listing_materials.listing_active='Y' 
        										ORDER BY specifications.specification";
       // echo $sql;
    ...
    function deleteListing($listingId, $deactReason="Deleted by supplier") {
        if ($deactReason) {
    ...
    function deleteListingFull($listingId) {
       // if ($deactReason) {
            $sql = "DELETE FROM `".$this->tblPrefix."listing_materials` WHERE id='".$listingId."'";
       // }
        //else {
           // return array('ERR','REASON_EMPTY');
       // }
...
supplier_id='".$customerId."' ";
    ...
    */
    function getAvgByThirdlevelCategory($thirdLevelCategoryId) {
    	...
    	$sql ="SELECT Specification_id,manufacturer_id,`category_id_2`,AVG(unit_cost) as average FROM `".$this->tblPrefix."listing_materials` 
    			WHERE category_id_2='".$thirdLevelCategoryId."' 
    					AND listing_active='Y' 
    					GROUP BY Specification_id,manufacturer_id ";
		
    	
    	/*$sql ="SELECT m.Specification_id, m.manufacturer_id, m.`category_id_2`,AVG(m.unit_cost) as average 
					FROM `".$this->tblPrefix."listing_materials` =`m` 
					WHERE m.category_id_2='352' 
					AND m.listing_active='Y' 
					
					AND id IN (
						SELECT `m`.`id`
						FROM `".$this->tblPrefix."listing_materials`=`m` , `".$this->tblPrefix."customers`=`c`, `".$this->tblPrefix."subscriptions`=`s`
						WHERE `m`.`supplier_id`=`c`.`customer_id` 
						AND `m`.`listing_active`='Y'
						AND `c`.`customer_status`='Y' 
						AND `c`.`customer_type`='S'
					...
					GROUP BY m.Specification_id, m.manufacturer_id";
    	*/
    	$result=$this->query($sql);
        // print_r($result);
        for ($i=0; $i<count($result); $i++) {
    ...
    */
        ...
        switch ($carlevel1) {
            ...
            case "2":
                $sql = "SELECT COUNT(`".$this->tblPrefix."services`.id) AS count  FROM `".$this->tblPrefix."services` 
                        ...
                        AND category_id_0='".$carlevel1."'"." AND `".$this->tblPrefix."subscriptions`.expire > '".time()."' 
                        		AND `".$this->tblPrefix."services`.status='Y' 
                        		AND `".$this->tblPrefix."subscriptions`.subscription_type = 'S' ";
                break;
            case "3":
    ...
    function getListingsforACategory_supply_2($carlevel1=1,$catlevel2='',$catlevel3='', $specification_id='',  $manufacturer_id='') {
    	
    	$sql ="SELECT COUNT(`lm`.`id`) as `count`
					FROM `".$this->tblPrefix."listing_materials`=`lm` , `".$this->tblPrefix."subscriptions`=`s` , `".$this->tblPrefix."customers`=`c`
					WHERE `lm`.`supplier_id` = `s`.`customer_id` 
					AND `lm`.`supplier_id` = `c`.`customer_id` 
					AND `lm`.`listing_active`='Y' 
					AND `s`.`expire`>'".time()."' 
					AND `s`.`subscription_type`='M' 
					AND `s`.`subscription_status`='Y' 
					AND `c`.`customer_status`='Y'";
    		
    		if($carlevel1){ $sql.="AND `lm`.`category_id_0` = '".$carlevel1."'  "; }
    		if($catlevel3){ $sql.="AND `lm`.`category_id_2` = '".$catlevel3."'  "; }
    		if($catlevel2){ $sql.="AND `lm`.`category_id_1` = '".$catlevel2."'  "; }
    		if($specification_id){ $sql.=" AND `lm`.`specification_id` = '".$specification_id."' "; }
    		if($manufacturer_id){ $sql.=" AND `lm`.`manufacturer_id` = '".$manufacturer_id."' "; }
    ...
    */
    ...
    function infoBox($boxIdentifier,$boxContent,$arrValues)
    {
        ...
        $arrReplace=array($arrValues['listCanAdd'],$arrValues['listTotal'],$arrValues['listInactive'],$arrValues['listActive'],$arrValues['listAllowed'],  $this->core->_SYS['CONF']['URL_MY_ACCOUNT'].'/my_subscriptions/?selections='.$arrValues['subsType'].'&packages=1#upgSubscript',);
    ...
    function getFeaturedListings($count = 1000) {
        $sql = "SELECT id,listing_header,supplier_id,image FROM `".$this->tblPrefix."listing_materials`";
        $sql .= " WHERE is_featured='Y' AND image IS NOT NULL ORDER BY id DESC ";
        $sql .= " LIMIT 0, $count";
