<?php
  ...
  Class Core{
      ...
      function  __Construct()
      {
       	...
       	*/
       		$devPath='diy_v0.5';	// folder path of development server - from the root
       		$demoPath='v0.5';	// folder path of demonsration server - from the root
         	...
         	if($IP_F[0]=='127'||$IP_F[0]=='172'||$IP_F[0]=='localhost')
         	{
                ...
                elseif($HOST=='www.fusissoft.co.uk' || $HOST=='fusissoft.co.uk'|| $HOST=='fusis.net'|| $HOST=='www.fusis.net'|| $HOST=='diy.fusis.net')
      ...
      public function msgBox($msgKey,$msg,$width="100%",$use=1)
      {
   	...
   	public function isAuthorized($use,$module='',$function='')
    {
   		switch ($use)
        {
            case 1:
            {
                ...
                if(!$module && !$this->authMatrix[$this->curSection()][$this->sessUType])
                {
                     /*
                      ...
                      */
                        header("location: ".$this->_SYS['CONF']['URL_FRONT'].'errors/?err=403&'.$this->curSection());
                }
                elseif($module)
                {
                    /*
                     ...
                     */
                    if($this->authMatrix[$module][$this->sessUType])
                    {
                        return true;
                    }
                    else
     ...
     */
     ...
     public function sysCheck()
     {
         ...
         // Create sql Class object
          try {require_once($this->_SYS['PATH']['CLASS_SQL']);
              $this->objSql = new Sql;
          } catch (Exception $e) {
     ...
     public function sysUpdate($key='',$value='')
     {
      ...
      function updateClient($clientUpdate, $customerId, $result, $dbValue)
      {
        if ($clientUpdate && $this->sessUId)
        {
            if (count($result) > 1)
            {
                ...
                $sqlUpdate = "UPDATE `".$this->_SYS['CONF']['PREFIX_TBL']."sys_support` SET guest_id='".$result[1]['guest_id']."',
content_wlist='".$dbValue."', content_search='".$result[1]['content_search']."',
time='".time()."', geo_info='". $val."' WHERE guest_id='".$result[0]['guest_id']."'";
                $resultUpdate = $this->objSql->query($sqlUpdate);
                if ($resultUpdate)
                {
                    $sqlDelete = "DELETE FROM `".$this->_SYS['CONF']['PREFIX_TBL']."sys_support` WHERE guest_id='".$result[1]['guest_id']."'
AND id='".$result[1]['id']."'";
...
FROM `".$this->_SYS['CONF']['PREFIX_TBL']."sys_support` WHERE client_id='".$customerId."'";
