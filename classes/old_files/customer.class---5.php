...
class Customer extends Sql {
    ...
    function setVariables($title, $fName, $lName, $email, $emailConfirm, $password, $passwordConfirm, $company, $address, $street, $city, $postal, $country, $phone, $fax, $mobile, $cusType, $latitude, $longitude, $subscription, $package) {
        $this->title = $title;
        $this->fName = $fName;
        $this->lName = $lName;
        $this->email = $email;
    ...
    function addSubscription($id, $cus_id,$changeType,$packages){
        
       $date_cur = date("Y-m-d");
       $date_cur_str = strtotime($date_cur);
               
        if($changeType==1){
            $subscription_type = 'M';
            $pack = explode("||",$packages);
            
            $date_ex = strtotime("+".$pack[1]." month", strtotime($date_cur));
            
            if($id){
                $sql ="UPDATE `".$this->tblPrefix."subscriptions` SET
                        `subscription_type` = '".$subscription_type."',
                        `package_type` = '".$pack[0]."',
                        `package_type_extend` = '".$pack[1]."',
                        `date` = '".$date_cur_str."',
                        `last_modified` = '".$date_cur_str."',
                        `expire` = '".$date_ex."',
                        `no_of_listings` = '',
                ...
                $sql = "INSERT INTO `".$this->tblPrefix."subscriptions` (`customer_id`, `subscription_type`, `package_type`, `package_type_extend`, `date`, `last_modified`, `expire`, `no_of_listings`, `subscription_status`, `recurring_profile_id`, `alert`)
                                                            VALUES ('".$cus_id."', '".$subscription_type."', '".$pack[0]."', '".$pack[1]."', '".$date_cur_str."', '".$date_cur_str."', '".$date_ex."', '', 'Y', NULL, 'Y')";
            }
        }else if($changeType==2){
            $subscription_type = 'S';
            
            $date_ex = strtotime("+".$packages." month", strtotime($date_cur));
            ...
            $sql = "INSERT INTO `".$this->tblPrefix."subscriptions` (`customer_id`, `subscription_type`, `package_type`, `package_type_extend`, `date`, `last_modified`, `expire`, `no_of_listings`, `subscription_status`, `recurring_profile_id`, `alert`)
                                                            VALUES ('".$cus_id."', '".$subscription_type."', '".$packages."', '', '".$date_cur_str."', '".$date_cur_str."', '".$date_ex."', '', 'Y', NULL, 'Y')";
        ...
        $sql_c = "INSERT INTO `".$this->tblPrefix."subscriptions` (`customer_id`, `subscription_type`, `package_type`, `package_type_extend`, `date`, `last_modified`, `expire`, `no_of_listings`, `subscription_status`, `recurring_profile_id`, `alert`)
                VALUES ('".$cus_id."', 'C', '1', NULL, '".$date_cur_str."', '".$date_cur_str."', '".$date_ex."', '', 'Y', NULL, 'Y')";
    
        $result = $this->query($sql_c);
    }
    
    function loadSubscription($cus_id){
        $sql = "SELECT `id`, `customer_id`, `subscription_type`, `package_type`, `package_type_extend`, `expire` 
            FROM `".$this->tblPrefix."subscriptions` WHERE `customer_id`='".$cus_id."' 
                AND (`subscription_type`='M' OR `subscription_type`='S')";
        
        $result = $this->query($sql);
        for($i=0;$i<count($result);$i++) {
            $list[$i][]=$result[$i]['id'];
            $list[$i][]=$result[$i]['customer_id'];
            $list[$i][]=$result[$i]['subscription_type'];
            $list[$i][]=$result[$i]['package_type'];
            $list[$i][]=$result[$i]['package_type_extend'];
            $list[$i][]=$result[$i]['expire'];
        }
    ...
    function addAdmin($cusType) {
        ...
        elseif ($passwordError[1] == true) {
            ...
            if ($result) {
                ...
                $expire = strtotime('+3 month', time());
                $this->setSubcriptons($customerId, $this->subscription, $this->package,$expire,'admin');
                $objEmail = new Email();
...
WHERE customers.customer_id='".$customer_id."'";
    ...
    function getCustomerData_2($customer_id) {
        $sql = "SELECT customers.f_name, customers.l_name, customers.company, customers.address, customers.customer_type,
     customers.street, customers.city, customers.postal, customers.country, customers.telephone, customers.fax,
customers.mobile, customers.email, customers.show_website, customers.title,   customers.latitude, customers.longitude,customers.id, customers.website, customers.mon,customers.sat,customers.sun,
subscriptions.subscription_type, subscriptions.package_type FROM `".$this->tblPrefix."customers`
customers LEFT OUTER JOIN `".$this->tblPrefix."subscriptions` subscriptions ON customers.customer_id=subscriptions.customer_id
WHERE customers.customer_id='".$customer_id."'";
        ...
        for($i=0;$i<count($result);$i++) {
            ...
            $list[$i][]=$result[$i]['website'];//21
            $list[$i][]=$result[$i]['show_website'];//22
            $list[$i][]=$result[$i]['customer_type'];//23  customer type
        }
    ...
    function updateSubcriptons($customerId, $subscription, $package,$expire,$packageExtend='',$profileId='',$time) {
...
//                        }
                ...
                ;
            case 'S': {
                    $sqlSelect = "SELECT subscription_type, expire, no_of_listings FROM `".$this->tblPrefix."subscriptions` 
                        WHERE customer_id='".$customerId."' AND subscription_type='S'";
                    $resultSelect = $this->query($sqlSelect);
...
//						}
                        $sql = "INSERT INTO `".$this->tblPrefix."subscriptions` (`customer_id`, `subscription_type`, `package_type`, `date`, `last_modified`, `expire`, `no_of_listings`, `subscription_status`,`recurring_profile_id`) 
                            VALUES ('".$customerId."', 'S', '".$time."', '".time()."', '".time()."', '".$expire."', '0', 'Y','".$profileId."')";
                    }
                }
        }
        $result = $this->query($sql);
    ...
    function setSubcriptons($customerId, $subscription='C', $package='',$expire='',$flag='') {
        ...
        if ($result[0]['COUNT(*)'] == 0) {
            ...
            $sql = "INSERT INTO `".$this->tblPrefix."subscriptions` (`customer_id`, `subscription_type`, `package_type`, `date`, `last_modified`, `expire`, `no_of_listings`, `subscription_status`) VALUES ('".$customerId."', '".$subscription."', '".$package."', '".time()."', '', '".$expire."', '".$no_of_listings."', '".$status."')";
            if ($this->query($sql)) {
                if ($subscription == 'C') {
