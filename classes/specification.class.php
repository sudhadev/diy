...
WHERE specifications.category_id_0='".$categoryId."' AND specifications.requested_by='".$customerId."' ORDER BY specifications.added_time DESC";
	...
	function get_cust_email_pendin_app($id){ /* add by maduranga for specification approval */
		
		$result=$this->query("SELECT cus.email FROM `".$this->tblPrefix."specifications` spec , `".$this->tblPrefix."customers` cus WHERE spec.`id` = '".$id."' AND spec.requested_by=cus.customer_id");
