...
`".$this->tblPrefix."services` ".$where."";
            ...
            if ($result)
            {
                for($i=0;$i<count($result);$i++)
                {
                    ...
                    $list[$i][]=$result[$i]['supplier_id'];//19
                }
                return $list;
            }
        }
      ...
      */
        ...
        {//$this->dev=true;
          $sql = "SELECT COUNT(*) as count FROM `".$this->tblPrefix."services` WHERE status='Y' AND category_id_0='".$topLevel."'";       		         	/*        	$sql = " SELECT COUNT(*) AS count  FROM `".$this->tblPrefix."services`                        INNER JOIN `".$this->tblPrefix."subscriptions`                        ON `".$this->tblPrefix."services`.supplier_id = `".$this->tblPrefix."subscriptions`.customer_id                        WHERE                        `".$this->tblPrefix."subscriptions`.subscription_status = 'Y'                            AND `@diy_____subscriptions`.customer_id IN (                            SELECT `customer_id` FROM `".$this->tblPrefix."customers` WHERE `customer_status`='Y')                                AND category_id_0='".$topLevel."'"." AND `".$this->tblPrefix."subscriptions`.expire > '".time()."' AND `@diy_____services`.status='Y' ";        		*/        		
        ...
        function getLatestServices(){
                  $where=" WHERE status = 'Y' ORDER BY added_date DESC LIMIT 0,10";
                  $list = $this->dList($where);
            return $list;
        }
