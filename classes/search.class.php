...
class Search extends Sql{
   	...
   	{ 
        ...
        if($keyword)
		{
                        ...
                        $naturalSearchStr = "SELECT `id` FROM `" . $this->tblPrefix . "tbl_natural_search` WHERE MATCH (specification,keywords) AGAINST ('+" . $searchKeyLine . "' IN BOOLEAN MODE)";
                        $result = $this->query($naturalSearchStr);
                        $sqlLIKE = " specifications.id IN (";
                        for ($i = 0; $i < count($result); $i++) {
                            $searchID = $result[$i]['id'];
                            $sqlLIKE.=$searchID . ",";
                        }
                        $sqlLIKE.="0)";
   	...
   	{ 
			...
			if ($searchKeyword)
			{
				$generatedSql = $this->generateSql($searchKeyword, $latitude, $longitude, $radius,'Y');
				...
				$list = $this->getSuppliers($x, $y, $z, $pg, '', '', '', $searchKeyword, $latitude, $longitude, $radius,'','Y');
				if($list)
				{
				...
				{
		    $generatedSql = $this->generateSql($checkedKeyword, $latitude, $longitude, $radius, 'Y');
                    $sqlCount = "SELECT categories.id, categories.category,'type' as refine_type, COUNT(*) ".$generatedSql." GROUP BY listing_materials.category_id_2 UNION SELECT specifications.id, specifications.specification, 'specification' as refine_type, COUNT(*) ".$generatedSql." AND listing_materials.category_id_2='".$x."' GROUP BY listing_materials.specification_id UNION SELECT manufacturers.id, manufacturers.manufacturer, 'manufacturer' as refine_type, COUNT(*)".$generatedSql." AND listing_materials.category_id_2='".$x."' AND listing_materials.specification_id='".$y."' GROUP BY listing_materials.manufacturer_id";
                    $resultCount = $this->query($sqlCount);
                    $this->dev = true;
...
function distance_cal($lat1, $lon1, $lat2, $lon2) {
    /*
     * distance calculation function - Create by Maduranga
     */
    
    //echo "</br>--".$lat1.">".$lon1."||".$lat2.">".$lon2."</br>";
    if(($lat1==$lat2) && ($lon1==$lon2)){
    		return 0;
    }else  if(($lat1==0.0000 && $lon1==0.0000) || ($lat2==0.0000 && $lat2==0.0000)){
        	return 0;
    }  else {
...
function getDistance_ni_moreInfo( $cid, $latitude, $longitude){ /// get distance for more details page - add by maduranga
	
	$sql = "SELECT `latitude`, `longitude` FROM `". $this->tblPrefix ."customers` WHERE `customer_id` = '".$cid."' ";
	$result = $this->queryPg($sql);
	
	//echo $this->distance_cal($latitude, $longitude, $result[0]['latitude'], $result[0]['longitude']);
	return $this->distance_cal($latitude, $longitude, $result[0]['latitude'], $result[0]['longitude']);
	
}
	

function getSuppliers_wishlist($categoryId, $specificationId, $manufacturerId, $pg=1, $resultsPerPage='', $url='', $orderBy='', $keyword='', $latitude='', $longitude='', $radius='', $listingId='',$subsStatus='',$preOrder='',$dir='')
{
	/*
	 * this is for only wishlist - copy of old getSuppliers function
	 * - Maduranga
	 * */
		...
		case '6':
			{
                ...
                default:{
                    $sentence = 'listing_materials.unit_cost ASC';
                }
                        break;
	}
	...
	$sql.="AND customers.customer_status = 'Y' ";
	...
	if ($result)
	{
		for($i=0;$i<count($result);$i++)
		{
										...
										$list[$i][]=  $this->distance_cal($latitude, $longitude, $result[$i]['latitude'], $result[$i]['longitude']);/* distance calculation change by maduranga*/
		...
		function getSuppliers($categoryId, $specificationId, $manufacturerId, $pg=1, $resultsPerPage='', $url='', $orderBy='', $keyword='', $latitude='', $longitude='', $radius='', $listingId='',$subsStatus='',$preOrder='',$dir='')
		{
         ...
         $earthRadius = $this->getEarthRadius();
           ...
           // $sql.= ",manufacturers.manufacturer ,  AVG(listing_materials.unit_cost) as average_price ";
            ...
            if($subsStatus){
                $sql.="AND subscriptions.expire> '".time()."' ";
            }
                
            $sql.="AND customers.customer_status = 'Y' ";
          //  $sql.="AND customers.customer_id IN ( SELECT `customer_id` FROM `".$this->tblPrefix."subscriptions` WHERE `expire`>'".time()."') ";
                            ...
                            $searchKeyword = $checkedKeyword;
                                    ...
                                    if ($searchKeyword)
                                    {
                                        ...
                                        $arrKeyWords=explode(" ",$keyword); 
                                         ...
                                         $naturalSearchStr = "SELECT `id` FROM `" . $this->tblPrefix . "tbl_natural_classified_ads_search` WHERE MATCH (keywords) AGAINST ('+" . $searchKeyLine . "' IN BOOLEAN MODE)";    
                                         $result = $this->query($naturalSearchStr);
                                                $sqlLIKE = " classified_ads.id IN (";
                                                for ($i = 0; $i < count($result); $i++) {
                                                    $searchID = $result[$i]['id'];
                                                    $sqlLIKE.=$searchID . ",";
                                                }
                                        ...
                                        /*************for new search- edit by maduranga****************/
                                                /*
                                            $sqlCount = "SELECT categories.id, categories.category, 
                                                COUNT(*) FROM `".$this->tblPrefix."classified_ads` classified_ads 
                                                    JOIN `".$this->tblPrefix."customers` customers ON classified_ads.supplier_id=customers.customer_id 
                                                        JOIN `".$this->tblPrefix."categories` categories ON classified_ads.category_id_1=categories.id 
                                                            WHERE classified_ads.keywords LIKE '%".$searchKeyword."%' 
                                                                AND customers.latitude<='".$points[0]."' 
                                                                    AND customers.latitude>='".$points[1]."' AND customers.longitude<='".$points[2]."' AND customers.longitude>='".$points[3]."' AND classified_ads.status='Y' GROUP BY classified_ads.category_id_1";
                                            */
                                            $sqlCount = "SELECT categories.id, categories.category, 
                                                COUNT(*) FROM `".$this->tblPrefix."classified_ads` classified_ads 
                                                    JOIN `".$this->tblPrefix."customers` customers ON classified_ads.supplier_id=customers.customer_id 
                                           ...
                                           // $list = $this->getClassifiedList($keyword, $latitude, $longitude, $radius, $pg=1, $categoryId, $url, $resultsPerPage, $orderBy);
                                            $list = $this->getClassifiedList_new2($keyword, $latitude, $longitude, $radius, $pg=1, $categoryId, $url, $resultsPerPage, $orderBy);
                                            /*
                                             * change for new search -Edit by maduranga
                                             */
                                            if ($list)
                                            {
                                                    return array('refineData' => $resultCount, 'searchData' => $list, 'correctWord' => $checkedKeyword, 'parameters'=>$categoryId);
     ...
     function getClassifiedList_new($keyword='', $latitude='', $longitude='', $radius='', $pg=1, $categoryId, $url='', $resultsPerPage='', $orderBy='', $listingId='')
		{
				...
				$sql.=", ".$earthRadius." * 2 * ASIN(SQRT(POWER(SIN((".$latitude." - abs(customers.latitude)) * pi()/180 / 2), 2) +  COS(".$latitude." * pi()/180 ) * COS(abs(customers.latitude) * pi()/180) *  POWER(SIN((".$longitude." - customers.longitude) * pi()/180 / 2), 2) )) as  distance ";
                 ...
                 $arrKeyWords=explode(" ",$keyword); 
                       foreach($arrKeyWords as $thisKey)
                       {
                        ...
                        //$naturalSearchStr = "SELECT `id` FROM `" . $this->tblPrefix . "tbl_natural_classified_ads_search` WHERE MATCH (keywords) AGAINST ('+" . $searchKeyLine . "' IN BOOLEAN MODE)";
                        $naturalSearchStr = "SELECT `id` FROM `" . $this->tblPrefix . "tbl_natural_classified_ads_search`  WHERE `keywords` LIKE '%".$searchKeyLine."%'";
                        $result = $this->query($naturalSearchStr);
                        $sqlLIKE = " classified_ads.id IN (";
                        for ($i = 0; $i < count($result); $i++) {
                            $searchID = $result[$i]['id'];
            ...
            if ($keyword)
            {
                ...
                $sql.= " AND customers.latitude<='".$points[0]."' AND customers.latitude>='".$points[1]."' AND customers.longitude<='".$points[2]."' AND customers.longitude>='".$points[3]."' AND ";
            }
        ...
        function getClassifiedList_new2($keyword='', $latitude='', $longitude='', $radius='', $pg=1, $categoryId, $url='', $resultsPerPage='', $orderBy='', $listingId='')
		{
           // $this->dev=TRUE;
        ...
        // $sql.= " AND customers.latitude<='".$points[0]."' AND customers.latitude>='".$points[1]."' AND customers.longitude<='".$points[2]."' AND customers.longitude>='".$points[3]."' AND ";
                       ...
                       $sql.= "classified_ads.id IN (".$naturalSearchStr.") 
                    AND customers.latitude<='".$points[0]."' AND customers.latitude>='".$points[1]."' AND customers.longitude<='".$points[2]."' AND customers.longitude>='".$points[3]."' AND ";
            
                
            }
            //$sql.= "(classified_ads.category_id_1='".$categoryId."' OR classified_ads.category_id_2='".$categoryId."') AND classified_ads.status='Y' ";
            $sql.= " classified_ads.status='Y' ";
...
function getClassifiedList($keyword='', $latitude='', $longitude='', $radius='', $pg=1, $categoryId, $url='', $resultsPerPage='', $orderBy='', $listingId='')
            ...
            if ($searchKeyword)
                ...
                /***** for new search -Edit by Maduranga  ***************************/
                       ...
                       foreach($arrKeyWords as $thisKey)
                       {
                        ...
                        $naturalSearchStr = "SELECT `id` FROM `" . $this->tblPrefix . "tbl_natural_services_search` WHERE MATCH (keywords) AGAINST ('+" . $searchKeyLine . "' IN BOOLEAN MODE)";
                        $result = $this->query($naturalSearchStr);
                        $sqlLIKE = " services.id IN (";
                        for ($i = 0; $i < count($result); $i++) {
                            $searchID = $result[$i]['id'];
                            $sqlLIKE.=$searchID . ",";
                        }
               ...
               /*
            	$sqlCount = "SELECT categories.id, categories.category, 
                    COUNT(*) FROM `".$this->tblPrefix."services` services 
                        JOIN `".$this->tblPrefix."customers` customers ON services.supplier_id=customers.customer_id 
                            JOIN `".$this->tblPrefix."categories` categories ON services.category_id_1=categories.id 
                                WHERE services.keywords LIKE '%".$searchKeyword."%' 
                                    AND customers.latitude<='".$points[0]."' AND customers.latitude>='".$points[1]."' AND customers.longitude<='".$points[2]."' AND customers.longitude>='".$points[3]."' AND services.status='Y' GROUP BY services.category_id_1";
                ...
                */
            	...
            	$list = $this->getServiceList_new($keyword, $latitude, $longitude, $radius, $pg, $categoryId, $url, $resultsPerPage, $orderBy);
                /*
                 * change for new search - edit by Maduranga
                 */
            	if ($list)
         ...
         */
         ...
         function getServiceList_new($keyword='', $latitude='', $longitude='', $radius='', $pg=1, $categoryId, $url='', $resultsPerPage='', $orderBy='', $listingId='')
		{
            // $this->dev=true;
            $earthRadius = $this->getEarthRadius();
            $points = $this->getPoints($latitude, $longitude, $radius);
         ...
         /*    $sql = "SELECT categories.category, 
                 ...
                 customers.city, 
                 customers.postal, ".$earthRadius." * 2 * ASIN(SQRT(POWER(SIN((51.5001524 - abs(customers.latitude)) * pi()/180 / 2), 2) + COS(51.5001524 * pi()/180 ) * COS(abs(customers.latitude) * pi()/180) * POWER(SIN((-0.1262362 - customers.longitude) * pi()/180 / 2), 2) )) as distance, services.title, services.notes, services.price, services.image, services.call_out_charge, services.business_name, services.affiliations, services.contact_person, services.accreditation, services.website, services.image2, services.image3, services.image4, 
                     services.id FROM `".$this->tblPrefix."services` services 
                         JOIN `".$this->tblPrefix."customers` customers ON services.supplier_id=customers.customer_id 
                             JOIN `".$this->tblPrefix."categories` categories ON services.category_id_1=categories.id 
                                 WHERE ";*/
				/* change by maduranga for get level 3 category Jason req*/
				$sql = "SELECT categories.category,
                 customers.f_name,
                 customers.l_name,
                 customers.telephone,
                 ...
                 customers.postal, ".$earthRadius." * 2 * ASIN(SQRT(POWER(SIN((51.5001524 - abs(customers.latitude)) * pi()/180 / 2), 2) + COS(51.5001524 * pi()/180 ) * COS(abs(customers.latitude) * pi()/180) * POWER(SIN((-0.1262362 - customers.longitude) * pi()/180 / 2), 2) )) as distance, services.title, services.notes, services.price, services.image, services.call_out_charge, services.business_name, services.affiliations, services.contact_person, services.accreditation, services.website, services.image2, services.image3, services.image4,
                     services.id FROM `".$this->tblPrefix."services` services
                         JOIN `".$this->tblPrefix."customers` customers ON services.supplier_id=customers.customer_id
                             JOIN `".$this->tblPrefix."categories` categories ON services.category_id_2=categories.id
                                 WHERE ";
