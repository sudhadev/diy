...
if ($_REQUEST['deleteId']) {
    ...
    $output2 = $objCustomer->get_active_sup_ids( $str[0], $str[1] , $str[2] , '',$_REQUEST['time']);  /*  Get Active user list   */
    ...
    <fieldset  id="page-middle-middle-content">
        ...
        for($n=0;$n<count($output);$n++) {
            ...
            <? if(!empty($_REQUEST['time'])) { ?>
            	<td align="left">
            		<?php if(in_array($output[$n][10], $output2)){ ?>
            		...
            		<?php }?>
            		<?php /*echo trim($output[$n][12]);?> <?php echo trim($output[$n][13]); */?>
            		<?php echo trim($output[$n][18]);?>
            	</td>
            	<? } ?>
        ...
        for($n=0;$n<count($output);$n++) {
            ...
            ?>
                <tr class="row0">
                    ...
                    <? if(!empty($_REQUEST['time'])) { ?>
                    	<td align="left">
                    		<?php if(in_array($output[$n][12], $output2)){ ?>
		            			<img height="13" width="12" src="<?php echo $objCore->_SYS['CONF']['URL_ICONS_CONSOLE']; ?>/active.png" title="Active" alt="Active"/>
		            		<?php }else{?>
		            			<img height="13" width="12" src="<?php echo $objCore->_SYS['CONF']['URL_ICONS_CONSOLE']; ?>/inactive.png" title="Inactive" alt="Inactive"/>
		            		<?php }?>
		            		<?php echo trim($output[$n][16]);?>
		            	</td>
                    <? } ?>
                    <td align="left"><?php echo $output[$n][1];?></td>
                    <td align="right"><?php echo $output[$n][2];?></td>
                    <td align="right"><?php echo $output[$n][3];?></td>
                    <td align="left"><?php echo $output[$n][14]."&nbsp;".$output[$n][15];?></td>
                    <td align="center">
...
<div id="specification_list">
    <fieldset  id="page-middle-middle-content">
        ...
        for($n=0;$n<count($output);$n++) {
            ...
            <? if(!empty($_REQUEST['time'])) { ?>
            	<td align="left">
            				<?php /*if(in_array($output[$n][9], $output2)){ ?>
		            			<img height="13" width="12" src="<?php echo $objCore->_SYS['CONF']['URL_ICONS_CONSOLE']; ?>/active.png" title="Active" alt="Active"/>
		            		<?php }else{?>
		            			<img height="13" width="12" src="<?php echo $objCore->_SYS['CONF']['URL_ICONS_CONSOLE']; ?>/inactive.png" title="Inactive" alt="Inactive"/>
		            		<?php } */?>
            		<?php echo trim($output[$n][11]);?>&nbsp;<?php echo trim($output[$n][12]);?>
            	</td>
            <? } ?>
