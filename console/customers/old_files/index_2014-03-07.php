...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
	 <script type="text/javascript" language="javascript" src="<?php echo $objCore->_SYS['CONF']['URL_JS_FRONT']?>/jquery.min.js"></script>
       <?php if($_REQUEST['f']=="add"){ ?>
			<script type="text/javascript">/* Add by maduranga Jquery confliction */
			var jq = $.noConflict();
				jq(document).ready(function(){
					jq('#cfname').keyup(function(){ 
						var max=jq(this).attr('maxlength');
						var valLen=jq(this).val().length;
						//$('#cfname_text_block').text( valLen+'/'+max);
						if(valLen==max){
							jq('#cfname_text_block').html('<strong style="color: red">'+valLen+'/'+max+'</strong>');
						}else{
							jq('#cfname_text_block').html('<strong>'+valLen+'/'+max+'</strong>');
						}
					});
				});
