...
{ 
    ...
    if(inputId){
        ...
        thisNode = this; //linode=thisNode = thisNode.childNode.getElementsByTagName('LI')[0];
        

        if(this.tagName=='A')thisNode = this.parentNode.getElementsByTagName('IMG')[0];
        cPath = this.getAttribute('cPath');
        /*---------- calling every levels in category tree seperatelly ------------*/
        if(cPath != null)
        {
            selectCategoryLevel(cPath);
        }
    ...
    // var childNode=thisNode.childNode;
    var ul = parentNode.getElementsByTagName('UL')[0];
    var li=parentNode.getElementsByTagName('LI')[0];
    ulid=li.id;
    ...
    if(thisNode.src.indexOf(plusImage)>=0){
        ...
        if(useAjaxToLoadNodesDynamically){	// Using AJAX/XMLHTTP to get data from the server
            ...
            if(parentId){
                ...
                ajaxObjectArray[ajaxIndex].requestFile = ajaxRequestFile + '?parentId=' + parentId + '&treeLevels=' + treeLevels  + '&id=' + id + '&ulid=' + ul.id + '&section=' + section;
...
function addNewNode(nameOfNewNode,ulid)
{
    ...
    source = document.getElementById(ulid);alert(source.id);
...
function initTree(level,RequestFile)
{
    treeLevels = level;
    
    ajaxRequestFile = RequestFile +'bin/ajax/get_category_console.php';
    imageFolder = RequestFile +'images/icons/';
