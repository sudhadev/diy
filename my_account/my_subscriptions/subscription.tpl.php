...
<div id="right_bar_middle">
                    ...
                    <div id="outer_middle">
                        ...
                        <div id="text_area" class="common_text"><span style="color:red;">IMPORTANT : </span> If you want to subscribe to both Building Supplies and Building Services, the businesses must have the same address and contact details, as the details taken during registration are used for both businesses. If you have businesses with different addresses and contact details, you will need to register each business separately.
                            ...
                            <table width="652px" cellspacing="0" cellpadding="0" border="0" align="center">
                                <tbody>
                                    <?php 
                                    $show_Num_of_Listing = FALSE;
                                    for ($i=0; $i<count($subcriptionData); $i++) {
                                                      if ($subcriptionData[$i][2]=='M'){
                                                          $show_Num_of_Listing = TRUE;
                                                      }  elseif ($subcriptionData[$i][2]=='S') {
                                                          if ($subcriptionData[$i][6] && $temp['expire'][0] == 'E') {
                                                              $show_Num_of_Listing = TRUE;
                                                          }
                                                      }
                                    }
                                    ...
                                    <?php                                   
                                        ...
                                        $service_month=0;
                                                    for ($i=0; $i<count($subcriptionData); $i++) {
                                                        ...
                                                        /** Add by Maduranga - for subscription changes */
                                                        
                                                        $date_exp = date("Y-m-d",strtotime("-2 day", $subcriptionData[$i][5]));
                                                        
                                                        
                                                        if ($subcriptionData[$i][2]=='M'){
                                                            if ($subcriptionData[$i][7]){
                                                                if(strtotime(date("Y-m-d"))<strtotime($date_exp)){
                                                                    //echo strtotime(date("Y-m-d")).'<'.$subcriptionData[$i][5].'||';
                                                                    $supplier_month=$subcriptionData[$i][7];
                                                                }
                                                            }
                                                        }  elseif ($subcriptionData[$i][2]=='S') {
                                                            if ($subcriptionData[$i][3]){
                                                                if(strtotime(date("Y-m-d"))<strtotime($date_exp)){
                                                                    $service_month=$subcriptionData[$i][3];
                                                                }
                                                            }
                                                        }
                                                       // echo $supplier_month.'-'.$service_month;
                                                        /***********************************/ 
                                                        ...
                                                        // flag if supplier already subscribed to the meterials (supplies)
                                                          ...
                                                          if($subcriptionData[$i][2]=="S") $flagService=true;
                                                        ?>
    ...
    if($subcriptionData[$i][7]) echo " [".str_pad($subcriptionData[$i][7],2,"0",STR_PAD_LEFT)." Months]";?></div></td>
                                                        ...
                                                        <?php if($show_Num_of_Listing){ ?>
                                                        <td></td>
                                                        <td width="134" class="chagrs_grid_text">
                                                            <div align="left">
                                                            <?php 
                                                              if ($subcriptionData[$i][2]=='M'){
                                                                  if ($subcriptionData[$i][6] && $temp['expire'][0] != 'E') echo $subcriptionData[$i][6]; elseif ($subcriptionData[$i][6] && $temp['expire'][0] == 'E') echo $subcriptionData[$i][6]." (Listings Exceeded)"; else echo "<I>none</I>"; 
                                                              }  elseif ($subcriptionData[$i][2]=='S') {
                                                                  if ($subcriptionData[$i][6] && $temp['expire'][0] != 'E') echo "1"; elseif ($subcriptionData[$i][6] && $temp['expire'][0] == 'E') echo " (Listings Exceeded)"; else echo "<I>none</I>"; 
                                                              }
                                                            ?>
                                                            </div>
                                                        </td>
                                ...
                                <form id="frmSubcriptions" name="frmSubcriptions" method="POST" action="<?php /* removed echo purposly*/  $sysURL."/my_account/payments/?listing=".$_REQUEST['listing']; ?>">
                                    ...
                                    <div id="option_inerform">
                                        <div class="option_form_selections">
                                            <label>
                                                <input name="selections" id="materials" value="M" onclick="selectSubOpt('M');"  type="radio" <?php echo ($subcriptionData[0][2]=='M')? 'checked="true"': ''; ?>>
                                            </label>
                                            Supplies</div>
...
$subStyle=array('light','dark');
                                                                ...
                                                                <td class="<?php echo strtolower($arraySubs[$col]);?>_<?php echo $subStyle[$row%2]?>"><label>
                                                                            ...
                                                                            <?php
                                                                                    if($arrSuppliesOptions[$row][$col][2]) {
                                                                                        if((int)str_pad($arrSuppliesOptions[$row][$col][5],2,"0",STR_PAD_LEFT)!=(int)$supplier_month){
                                                                                            ...
                                                                                            echo '<div style="margin-right: 5px; width: auto; float: left; margin-left: 5px;">'.$arrIcons['EXPIRED'].'</div>';
                                                                                        }
                                        ...
                                        <div class="option_form_selections">
                                            <label>
                                                <input name="selections" id="services" value="S" onclick="selectSubOpt('S');" type="radio" <?php echo ($subcriptionData[0][2]=='S')? 'checked="true"': ''; ?>>
                                            </label>
                                            Services</div>
