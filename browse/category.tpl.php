		                  	...
		                  	(count($listSub)>$numOfSubCatsInList)? $loopTill=$numOfSubCatsInList-1: $loopTill=$loopTill;
                               ...
                               <?php     }break;
                                    ...
                                    { // change by maduranga for product list count issue 
                                 		$p_count=$objListing->getListingsforACategory_supply_2($topList[$tl]['id'],$list[$l][0],$listSub[$d][0]);
                                 	?>
                                        <li><a href="?f=spec&tcid=<?php echo $topList[$tl]['id'];?>&pcid=<?php echo $list[$l][0];?>&catId=<?php echo $listSub[$d][0];?>" ><?php echo $listSub[$d][3];?></a><?php if($listSub[$d][0]>0) { ?> &nbsp;[<?php echo $p_count;?>]<?php }?></li>
                               <?php     }break;
                                }
                                }?>
		                  </ul>
