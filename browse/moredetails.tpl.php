...
if(!is_object($objWishList))
{
    $objWishList = new WishList($objCore->gConf);
 	...
 	$customerInfo = $customerData[0];
		...
		$listing_spec_id = $objListing->dList_spec("WHERE id=" . $list[0][4] . "");
        ...
        <tr>
			...
			<div class="configure_quot_tabs">
                ...
                <span style="font-family: Arial; text-align: left;float: left;padding: 3px;">
                    <span style="font-size: 13px;font-weight: bold;text-align: justify;"><?php if($list[0][21]){ echo ($list[0][21]); }else{ echo trim($listing_spec_id[0][3]);} ?></span><br/>
...
$onclck = "add('1');";
                        ...
                        ?>
        <img onclick="<?php echo $onclck; ?>" src="<?php echo $objCore->_SYS['CONF']['URL_IMAGES_FRONT'];?>/add-to-wish-list.png"/><!-- fb and twitt start--><?php 
$currentUrl="http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";echo ' ';
?>
<div class="fb-share-button" data-href="<?php $currentUrl;?>" data-layout="button_count"></div>
<a href="https://twitter.com/share" class="twitter-share-button" data-count="horizontal" data-text="DIY:">Tweet</a>
...
(document, 'script', 'twitter-wjs');</script>
        ...
        <div id="error_msg" style="width:350px; margin-left:0px">
            ...
            <table width="300" class="common_text">
            	...
            	?>
                <?php //if($_REQUEST['dis']=="" || $_REQUEST['dis']=="NAN"){}else{ 
                        
                     if($_REQUEST['dis']!=""  || $distance_claculate){   ?>
                <tr>
                    <td width="110"><strong>Distance</strong>
                        ...
                        <td><?php 
                        	if($distance_claculate){ echo $distance_claculate; }
                        	else{ echo $_REQUEST['dis']; }
                        ?>
                        &nbsp;<?php echo $objCore->gConf['SEARCH_UNIT'];?>
                        </td>
                </tr>
                <?php } ?>
                <tr>
                    ...
                    <td><strong>Delivery Cost</strong>
                        ...
                        <td>
                        	<?php 
                        		...
                        		if($list[0][9]=='Free' || $list[0][9]=='0'){ echo 'Free UK'; }
                        		else if($list[0][9]=='Ring'){ echo 'Ring for details'; }
                        		else { echo ' £ '.$list[0][9];}
                        	?>
                        </td>
                </tr>
            ...
            <iframe src="image-gallery.php?catid=1&lid=<?php echo $list[0][0]; ?>&cid=<?php echo $_REQUEST['cid']; ?>" width='320px' height='400px' frameborder='0' style="margin-top: -7px;" scrolling="no" id="fbtake"></iframe>
            </td>
...
//-->
document.getElementById("fbtake").onload=function() {

	var x = document.getElementById("fbtake");
	var yy = x.contentDocument;
	var yyyshreimg=yy.getElementById("fbshairimg").src;
	/* document.getElementsByTagName('head')[0].appendChild('<meta property="og:image" content="'+yyyshreimg+'"  >'); */
