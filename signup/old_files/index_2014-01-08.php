...
if($objCore->sessCusId!=$customerDetails[0][0])   //If the customer is logged in with a different correct account
...
else      // IF the customer logged in with the rlevant accout Redirect to the subscription selection page   
   ...
   header('Location:'.$objCore->_SYS['CONF']['URL_MY_ACCOUNT'].'/first_login/?f=select_subscription&promo_code='.$_REQUEST['code'].'&promo_key='.$_REQUEST['key'].'&cusType='.$_REQUEST['cusType']);
    ...
    header('Location:'.$objCore->_SYS['CONF']['URL_FRONT'].'signup/?email='.$_REQUEST['email']);
}
 //if($_REQUEST['ex']=="yes"){
}
  //}
...
// 	}
         if($_REQUEST['f'] == "verify"){
            ...
            if(!isset($_REQUEST['ver_code'])){
                ...
                if($msg[0]=="SUC"){
                    ...
                    //header('Location:'.$objCore->_SYS['CONF']['URL_LOGIN_FRONT'].'?err=530&verify=suc');
	...
	{
            //print_r($_REQUEST);
            $objRegistration= new Registration($objCore->gConf,$_POST['title'], $_POST['fName'], $_POST['lName'], $_POST['email'], $_POST['emailConfirm'], $_POST['password'], $_POST['confirmPassword'], $_POST['company'], $_POST['address'], $_POST['street'], $_POST['city'], $_POST['postcode'], $_POST['country'], $_POST['phone'], $_POST['fax'], $_POST['mobile'], $_POST['cusType'], $_POST['confirmedLatitude'], $_POST['confirmedLongitude'], $subscription, $package);
...
<?php require_once($objCore->_SYS['PATH']['CLASS_GEO']); //Making a referance to Geo Class 
